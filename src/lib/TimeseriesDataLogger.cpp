...
namespace fmc {
    TimeseriesDataLogger::TimeseriesDataLogger(
        ...
        for (auto& [signal, mapped_name] : logging_config.items()) {
            ...
            else if (val_type == rttr::type::get<std::reference_wrapper<const int64_t>>()) {
                this->int_signals_to_log.emplace(mapped_name, val_variant.get_value<std::reference_wrapper<const int64_t>>());
            }
            else {
        ...
        if (!this->int_signals_to_log.empty()) {
            csv_header += "," + this->csv_header_string<int64_t>(this->int_signals_to_log);
        }
    ...
    void TimeseriesDataLogger::log() {
        ...
        if (!this->int_signals_to_log.empty()) {
            out += "," + this->signals_to_log_to_string<int64_t>(this->int_signals_to_log);
        }
